        ...
        for batch_idx,(x,y, _, _) in enumerate(train_loader):
            ...
            if batch_idx % log_interval == 0:
                ...
                for t in range(seq_length):
                    ...
                    batch_ori_err = batched_ori_err(output_t, y, device)
        ...
        with torch.no_grad():
            ...
            for t in range(seq_length):
                ...
                for batch_idx,(x,y, _, _) in enumerate(val_loader):
                    ...
                    total_valid_ori_error.append(batched_ori_err(output_t, y, device))
...
if __name__ == '__main__':
    ...
    run = wandb.init(project="screwing_estimation", entity="serialexperimentsleon", group=group_id, job_type='train and validate',
    config={
    ...
    bag_idxs = np.asarray(range(num_eps))
    np_seed = 0
    rng = np.random.default_rng(np_seed)
    wandb.config.update({'np_seed': np_seed})
    rng.shuffle(bag_idxs)
    split = int(np.floor(train_ratio * num_eps))
