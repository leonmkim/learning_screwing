...
class ContactDataset(Dataset):
    ...
    def __init__(self, bag_path, window_size, obj_name, in_cam_frame=True, im_resize=None, centered=False, load_in_mem=True):
        ...
        self.im_shape = cv2.imread(self.im_paths[0]).shape[:2]
        ...
        assert self.depth_num_msgs == len(self.depth_times), 'bag depth num msgs does not match depth timestamp length!'
        ...
        if load_in_mem:
            ## read the label
            self.contact_df = pd.read_pickle(os.path.join(bag_path.strip('.bag'), 'proprio_df.pkl'))
    ...
    def __getitem__(self, idx):
        ...
        nrst_tfs_np = self.proprio_df.iloc[nrst_proprio_idxs][self.pose_topics].values
        nrst_poses_np = []
        ...
        ## get wrenches
        nrst_wrench_np = self.proprio_df.iloc[nrst_proprio_idxs][self.wrench_topics].values
        nrst_wrench_np = []
        ...
        contact_list, contact_times, contact_time_diffs = self.get_contacts(contact_idx, self.contact_filtered_df)
        ...
        for contact in contact_list:
            if not contact: # if not a null contact
